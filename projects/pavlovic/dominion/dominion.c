...
// 3) salvager
int _salvager(struct gameState *state, int choice1, int currentPlayer, int handPos) {
	//+1 buy
	state->numBuys++;
...
// 4) sea_hag
int _sea_hag(struct gameState *state, int currentPlayer) {
	int i;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus) {
	...
	// z=z-1;
	//   }
	    status = _adventurer(drawntreasure, state, currentPlayer, cardDrawn, temphand, z);
	    return status;
	...
	//   discardCard(handPos, currentPlayer, state, 0);
	//   return 0;
		status = _smithy(currentPlayer, state, handPos);
		return status;
