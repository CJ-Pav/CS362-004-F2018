...
int main() {
    ...
    int position = 0, cA = 0, cB = 0, cC = 0, bonus = 0;
    int seed = 1000;
    int numPlayers = 3;
    int thisPlayer = 0;
	...
	printf("Only failures will print.\n");
    for(i = 0; i < TESTRUNS; i++) {
        printf("Test: %d\nCard: %s\n", i, TESTCARD);

        numPlayers = randomInt(numPlayers) + 2;
        init(numPlayers, k, seed, &G);

        // Smithy receives 3-cards
        // Dup game state

        // Add cards to deck, hand and discard vary w/ # of players
        ...
        if (position < G.handCount[player]) {
            G.hand[thisPlayer][position] = adventurer;
        } else {
        ...
        if (drawnCards == (G.deckCount[thisPlayer] + G.discardCount[thisPlayer])) {
            ...
            cardEffect(adventurer, cA, cB, cC, &testG, position, &bonus);


        int handCount = testG.handCount[thisPlayer];
            ast(testG.handCount[thisPlayer] == G.handCount[thisPlayer] + newCards - discarded,
                         "Receives 2 cards", &G,
                         &testG);
            ast(testG.deckCount[thisPlayer] == G.deckCount[thisPlayer] - drawnCards + shuffledCards,
                         "Deck Less Card", &G, &testG);

            if(DEBUG){printf("First Treasure %d ", treasure[0].type);}
            if(DEBUG){printf("Second Treasure %d ", treasure[1].type);}
        if(DEBUG){printf("Real First Treasure %d\n ", testG.hand[thisPlayer][handCount - 1]);}
        if(DEBUG){printf("Real Second Treasure %d\n ", testG.hand[thisPlayer][handCount - 2]);}
            ast(testG.hand[thisPlayer][handCount - 1] == treasure[1].type, "First Treasure", &G, &testG);
            ast(testG.hand[thisPlayer][handCount - 2] == treasure[0].type, "Second Treasure", &G, &testG);

            if (shuffledCards) {
                ast(testG.discardCount[thisPlayer] == 0,
                             "Discard Pile has shuffle", &G, &testG);
            } else {

                ast(testG.discardCount[thisPlayer] == G.discardCount[thisPlayer] + drawnCards - newCards, "Discard", &G, &testG);
            }

            ast(testG.playedCardCount = G.playedCardCount + drawnCards, "Played Card Count", &G, &testG);
            ast(testG.coins == G.coins + xtraCoins, "No extra coins received", &G, &testG);
            ast(testG.whoseTurn == G.whoseTurn, "Same Players Turn", &G, &testG);
            ast(testG.numActions == G.numActions + numActions, "Number of actions", &G, &testG);
            ast(testG.numBuys == G.numBuys, "Number of Buys", &G, &testG);
